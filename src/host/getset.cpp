                                                                ...
                                                                const til::point position) noexcept
{
    try
    {
        ...
        if (const auto io = gci.GetVtIo(&context))
        {
            ...
            io->WriteFormat(FMT_COMPILE("\x1b[{};{}H"), y, x);
        }
                                                                  ...
                                                                  const bool enableCmdShim) noexcept
{
    try
    {
        ...
        if (const auto io = gci.GetVtIo(&context))
        {
            ...
            RETURN_IF_FAILED(WriteConsoleOutputWImplHelper(context, infos, w, targetViewport, writtenViewport));

            infos.clear();
            infos.resize(a, CHAR_INFO{ fillCharacter, fillAttribute });
            RETURN_IF_FAILED(WriteConsoleOutputWImplHelper(context, infos, w, sourceViewport, writtenViewport));
        }
        else
        {
            auto& buffer = context.GetActiveBuffer();
